...
OUT_DIR = ROOT / "artifacts" / "strategy_50_2c_without_monthly_lock_20260629"
...
def main() -> None:
    ...
    variants = [
        ("no_lock_keep_quota", True),
        ("no_lock_no_quota", False),
    ]
    variant_payloads: dict[str, dict[str, Any]] = {}
    stress_rows: list[dict[str, Any]] = []

    for variant_id, keep_quota in variants:
        equity = _simulate_no_monthly_lock(
            base_side,
            trend_side,
            runner_side,
            weak_trend,
        ...
        monthly.to_csv(OUT_DIR / f"{variant_id}_monthly.csv", index=False)
        yearly.to_csv(OUT_DIR / f"{variant_id}_yearly.csv", index=False)
        variant_payloads[variant_id] = {
            "keep_quota_mode": keep_quota,
    ...
    selections.to_csv(OUT_DIR / "strategy_50_selections_from_2c.csv", index=False)
    stress.to_csv(OUT_DIR / "strategy_50_stress.csv", index=False)

    summary = {
        "status": "strategy_50_2c_without_monthly_lock_done",
        ...
        "change": "Reuse Strategy 2C logic, but disable the monthly profit halt/post-lock runner. Test with and without the pre-10-trade quota throttle.",
        "cost_model": {
        ...
        "decision": _decision(variant_payloads),
        "files": {
            "summary": _relpath(OUT_DIR / "summary.json"),
            "report": _relpath(OUT_DIR / "report.md"),
            "selections": _relpath(OUT_DIR / "strategy_50_selections_from_2c.csv"),
            "stress": _relpath(OUT_DIR / "strategy_50_stress.csv"),
        },
    }
    ...
    *,
    ...
    for start, end in zip(market["month_starts"], np.r_[market["month_starts"][1:], len(base_side)]):
        ...
        for index in range(start, end):
            ...
            if adverse_shock:
                if current_base_side and month_orders < lock_search.REQUIRED_MIN_MONTHLY_ORDERS:
                    current_side = current_base_side
...
def _decision(variants: dict[str, dict[str, Any]]) -> dict[str, Any]:
    rows = {key: value["row"] for key, value in variants.items()}
    return {
        "verdict": "MONTHLY_LOCK_IS_CRITICAL_FOR_2C",
        "reason": "去掉月度盈利后停手后，策略几乎一直满市场暴露，换手大幅上升，2025直接崩掉，最大回撤接近 -100%。",
        "hard_pass_variants": [key for key, row in rows.items() if row["hard_pass"]],
    }
...
def _render_report(summary: dict[str, Any]) -> str:
    ...
    for variant_id, payload in summary["variants"].items():
        row = payload["row"]
        lines.append(
            f"| `{variant_id}` | {row['return_2025_pct']:.2f}% | {row['return_2026_pct']:.2f}% | "
