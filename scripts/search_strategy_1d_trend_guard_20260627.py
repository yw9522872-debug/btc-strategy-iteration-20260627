...
OUT_DIR = ROOT / "artifacts" / "strategy_1d_trend_guard_20260627"
...
STRONG_TREND_GAP_BPS = 300.0
STRONG_TREND_ADX_MIN = 24.0
...
def main() -> None:
    ...
    equity = _simulate(base_side, trend_side, weak_trend, market, selections, cost_per_side=lock_search.COST_PER_SIDE)
    ...
    selections.to_csv(OUT_DIR / "strategy_1d_selections.csv", index=False)
    signals.to_csv(OUT_DIR / "strategy_1d_signals.csv", index=False)
    equity.to_csv(OUT_DIR / "strategy_1d_equity.csv", index=False)
    monthly.to_csv(OUT_DIR / "strategy_1d_monthly.csv", index=False)
    yearly.to_csv(OUT_DIR / "strategy_1d_yearly.csv", index=False)
    stress.to_csv(OUT_DIR / "strategy_1d_stress.csv", index=False)

    chart_files = _plot_trade_charts(equity)

    summary = {
        "status": "strategy_1d_trend_guard_ready",
        "strategy_id": STRATEGY_ID,
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "research_only": True,
        "not_a_freeze": True,
        "strict_no_future_function": True,
        "base": "strategy_1c_trend_runner_20260627",
        "change": (
            "Keep the fixed ret_state 64/100 signal, but cap leverage at 8x, block large trades "
            "against strong trend, and shrink new/reverse trades in weak trend zones to 0.10x."
        ),
        "rules": {
            "base_signal": {
                "family": "ret_state",
        ...
        "risk_flags": {
            ...
            "trend_guard_choice_is_posthoc_research": True,
        ...
        "hashes": {
            ...
            "signals_sha256": lock_search._sha256(OUT_DIR / "strategy_1d_signals.csv"),
        },
        "files": {
            "summary": _relpath(OUT_DIR / "summary.json"),
            "report": _relpath(OUT_DIR / "report.md"),
            "selections": _relpath(OUT_DIR / "strategy_1d_selections.csv"),
            "signals": _relpath(OUT_DIR / "strategy_1d_signals.csv"),
            "equity": _relpath(OUT_DIR / "strategy_1d_equity.csv"),
            "monthly": _relpath(OUT_DIR / "strategy_1d_monthly.csv"),
            "yearly": _relpath(OUT_DIR / "strategy_1d_yearly.csv"),
            "stress": _relpath(OUT_DIR / "strategy_1d_stress.csv"),
...
def _result_row(equity: pd.DataFrame, monthly: pd.DataFrame, yearly: pd.DataFrame) -> dict[str, Any]:
    return strategy_1c._result_row(equity, monthly, yearly)
...
def _diagnostics(equity: pd.DataFrame) -> dict[str, Any]:
    ...
    reason_counts = equity["guard_reason"].value_counts().to_dict()
    return {
...
def _plot_trade_charts(equity: pd.DataFrame) -> list[Path]:
    ...
    for year in ["2025", "2026"]:
        ...
        buy_sizes = _marker_sizes(buys["delta_position"].abs())
        sell_sizes = _marker_sizes(sells["delta_position"].abs())
        ...
        ax.set_title(f"1号D策略 {year} 年 BTC 15分钟回测买卖点", fontsize=18)
        ax.set_ylabel("BTC价格 USDT")
        ax.grid(True, linestyle="--", alpha=0.25)
        ax.legend(loc="upper left")
        summary_text = (
            f"买点 {len(buys)} 个；卖点 {len(sells)} 个；"
            "小点代表小仓位补交易，大点代表主仓变化"
        )
        ax.text(
            0.01,
        ...
        fig.tight_layout()
        path = OUT_DIR / f"strategy1d_trades_{year}.png"
        fig.savefig(path)
        plt.close(fig)
