...
OUT_DIR = ROOT / "artifacts" / "strategy_1g_cap7_selective_runner_20260627"
...
def main() -> None:
    ...
    selections.to_csv(OUT_DIR / "strategy_1g_selections.csv", index=False)
    signals.to_csv(OUT_DIR / "strategy_1g_signals.csv", index=False)
    equity.to_csv(OUT_DIR / "strategy_1g_equity.csv", index=False)
    monthly.to_csv(OUT_DIR / "strategy_1g_monthly.csv", index=False)
    yearly.to_csv(OUT_DIR / "strategy_1g_yearly.csv", index=False)
    stress.to_csv(OUT_DIR / "strategy_1g_stress.csv", index=False)
    chart_files = _plot_trade_charts(equity)

    summary = {
        "status": "strategy_1g_cap7_selective_runner_ready",
        ...
        "change": (
            "Keep the 1F selective runner and guards, but lower the main leverage cap from 8x to 7x."
        ),
        "rules": {
            "base_signal": {
                "family": "ret_state",
        ...
        "hashes": {
            ...
            "signals_sha256": lock_search._sha256(OUT_DIR / "strategy_1g_signals.csv"),
        },
        "files": {
            "summary": _relpath(OUT_DIR / "summary.json"),
            "report": _relpath(OUT_DIR / "report.md"),
            "selections": _relpath(OUT_DIR / "strategy_1g_selections.csv"),
            "signals": _relpath(OUT_DIR / "strategy_1g_signals.csv"),
            "equity": _relpath(OUT_DIR / "strategy_1g_equity.csv"),
            "monthly": _relpath(OUT_DIR / "strategy_1g_monthly.csv"),
            "yearly": _relpath(OUT_DIR / "strategy_1g_yearly.csv"),
            "stress": _relpath(OUT_DIR / "strategy_1g_stress.csv"),
...
def _plot_trade_charts(equity: pd.DataFrame) -> list[Path]:
    ...
    for year in ["2025", "2026"]:
        ...
        ax.set_title(f"1号G策略 {year} 年 BTC 15分钟回测买卖点", fontsize=18)
        ...
        fig.tight_layout()
        path = OUT_DIR / f"strategy1g_trades_{year}.png"
        fig.savefig(path)
        plt.close(fig)
