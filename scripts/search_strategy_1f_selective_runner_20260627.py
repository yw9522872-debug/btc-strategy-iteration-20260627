...
OUT_DIR = ROOT / "artifacts" / "strategy_1f_selective_runner_20260627"
...
def main() -> None:
    ...
    runner_side = _runner_side(features)
    ...
    stress = pd.DataFrame(
        [
            _stress_row(base_side, trend_side, runner_side, weak_trend, market, selections, 0.001, 0),
            _stress_row(base_side, trend_side, runner_side, weak_trend, market, selections, 0.0015, 0),
            _stress_row(base_side, trend_side, runner_side, weak_trend, market, selections, 0.002, 0),
            _stress_row(base_side, trend_side, runner_side, weak_trend, market, selections, 0.001, 1),
            _stress_row(base_side, trend_side, runner_side, weak_trend, market, selections, 0.0015, 1),
        ]
    )

    selections.to_csv(OUT_DIR / "strategy_1f_selections.csv", index=False)
    signals.to_csv(OUT_DIR / "strategy_1f_signals.csv", index=False)
    equity.to_csv(OUT_DIR / "strategy_1f_equity.csv", index=False)
    monthly.to_csv(OUT_DIR / "strategy_1f_monthly.csv", index=False)
    yearly.to_csv(OUT_DIR / "strategy_1f_yearly.csv", index=False)
    stress.to_csv(OUT_DIR / "strategy_1f_stress.csv", index=False)
    chart_files = _plot_trade_charts(equity)

    summary = {
        "status": "strategy_1f_selective_runner_ready",
        "strategy_id": STRATEGY_ID,
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "research_only": True,
        "not_a_freeze": True,
        "strict_no_future_function": True,
        "base": "strategy_1e_shock_guard_20260627",
        "change": (
            "Keep the 1E trend and shock guards, but make the post-lock runner more selective: "
            "only a tiny 0.10x runner after a stronger 700 bps trend condition persists."
        ),
        "rules": {
            "base_signal": {
                "family": "ret_state",
            ...
            "post_lock_runner": {
                "runner_gap_bps": RUNNER_GAP_BPS,
                "runner_adx_min": RUNNER_ADX_MIN,
                "runner_confirm_bars": RUNNER_CONFIRM_BARS,
                "runner_leverage": RUNNER_LEVERAGE,
                "runner_side": "stricter than the strong trend guard",
            },
        ...
        "hashes": {
            ...
            "signals_sha256": lock_search._sha256(OUT_DIR / "strategy_1f_signals.csv"),
        },
        "files": {
            "summary": _relpath(OUT_DIR / "summary.json"),
            "report": _relpath(OUT_DIR / "report.md"),
            "selections": _relpath(OUT_DIR / "strategy_1f_selections.csv"),
            "signals": _relpath(OUT_DIR / "strategy_1f_signals.csv"),
            "equity": _relpath(OUT_DIR / "strategy_1f_equity.csv"),
            "monthly": _relpath(OUT_DIR / "strategy_1f_monthly.csv"),
            "yearly": _relpath(OUT_DIR / "strategy_1f_yearly.csv"),
            "stress": _relpath(OUT_DIR / "strategy_1f_stress.csv"),
...
def _runner_side(features: pd.DataFrame) -> np.ndarray:
    trend_gap = features["trend_close_ema_gap_bps_60"].to_numpy(float)
    adx = features["trend_adx_30"].to_numpy(float)
    raw_side = np.zeros(len(features), dtype=np.int8)
    raw_side[(trend_gap >= RUNNER_GAP_BPS) & (adx >= RUNNER_ADX_MIN)] = 1
    raw_side[(trend_gap <= -RUNNER_GAP_BPS) & (adx >= RUNNER_ADX_MIN)] = -1
    if RUNNER_CONFIRM_BARS <= 1:
        return raw_side
    side = np.zeros(len(features), dtype=np.int8)
    running_side = 0
    running_bars = 0
    for index, current_side in enumerate(raw_side):
        if current_side and current_side == running_side:
            running_bars += 1
        else:
            running_side = int(current_side)
            running_bars = 1 if current_side else 0
        if current_side and running_bars >= RUNNER_CONFIRM_BARS:
            side[index] = current_side
    return side
...
def _plot_trade_charts(equity: pd.DataFrame) -> list[Path]:
    ...
    for year in ["2025", "2026"]:
        ...
        ax.set_title(f"1号F策略 {year} 年 BTC 15分钟回测买卖点", fontsize=18)
        ...
        fig.tight_layout()
        path = OUT_DIR / f"strategy1f_trades_{year}.png"
        fig.savefig(path)
        plt.close(fig)
